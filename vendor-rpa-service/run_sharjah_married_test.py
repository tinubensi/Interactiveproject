#!/usr/bin/env python3
"""
Test Sukoon bot with Sharjah emirate, India nationality, female, married, DOB 13-06-1996
"""
import asyncio
import json
import sys
...
async def main():
    ...
    # Load test lead data
    test_lead_path = Path(__file__).parent / 'test-leads' / 'sukoon-sharjah-married-test.json'
    ...
    try:
        ...
        async with SukoonBot(credentials=credentials, config=bot_config) as bot:
            ...
            output_file = Path(__file__).parent / f"sukoon-sharjah-married-scraped-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
