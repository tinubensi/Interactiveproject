"""
GIG Gulf Benefits Enricher Module

Enriches plans scraped from the portal with static benefits data.
Matches plans by name and intelligently merges benefit information.
"""

from typing import Dict, Any, Optional, List
from vendors.gig_gulf.benefits_data import get_benefits_for_plan, normalize_plan_name
import copy


def enrich_plan_with_benefits(plan: Dict[str, Any]) -> Dict[str, Any]:
    """
    Enrich a portal plan with static benefits data.
    ...
    # Get plan name - try multiple possible field names
    plan_name = plan.get('planName') or plan.get('plan_name')
    if not plan_name:
        return plan
...
def merge_benefit_sections(existing_details: Dict[str, Any], benefit_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Args:
        ...
        benefit_data: Static benefits data from GIG plans
