...
class Gig_gulfBot(InsuranceBot):
    ...
    async def fill_insurance_form(self, form_data: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.debug("Selecting Effective Date...")
            await page1.locator("#MindateBlock").click()
            await asyncio.sleep(0.5)
            # Select a date from the date picker (e.g., 24th)
            try:
                await page1.get_by_role("cell", name="24").click()
                self.logger.debug("Selected date 24 from picker")
            except:
                # Fallback: try clicking any available date
                try:
                    await page1.get_by_role("cell", name="23").click()
                    self.logger.debug("Selected date 23 from picker (fallback)")
                except:
                    self.logger.warning("Could not select date from picker")
            await asyncio.sleep(0.3)
            
            # Fill Date of Birth (MaxdateBlock) - Use type for masked input
            self.logger.debug("Filling Date of Birth...")
            ...
            self.logger.debug("Filling phone field...")
