...
class AlsagrAdapter:
    ...
    def normalize_response(self, plans: List[Dict[str, Any]], lead_id: str) -> List[Dict[str, Any]]:
        ...
        for plan in plans:
            # Add lead ID if not present
            if 'leadId' not in plan:
                plan['leadId'] = lead_id
...
def transform_lead_to_alsagr_format(lead_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    phone_raw = lead_data.get('phone', {}).get('number', '')
    ...
    # First try lobData.dependents (old format - for backward compatibility with test files)
    ...
    # Then try formData sections (new format from production UI)
    if not dependents:
        for key, value in form_data.items():
            if key.startswith('section-') and isinstance(value, list):
                # Found the members section
                for member in value:
