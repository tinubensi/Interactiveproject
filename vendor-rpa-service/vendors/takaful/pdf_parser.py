...
Matches Alsagr PDF parser structure
"""
import pdfplumber
import re
import os
...
async def parse_plan_pdf(pdf_path: str, base_plan_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Parses the Takaful plan PDF and enriches the base plan data.
    Matches Alsagr PDF parser structure exactly.
    
    Args:
        pdf_path: Path to the downloaded PDF file
        base_plan_data: Existing plan data from table extraction
    ...
    try:
        ...
        # Extract Waiting Period (same logic as Alsagr)
        ...
        # Extract Structured Inpatient & Outpatient Coverage (same as Alsagr)
        inpatient_benefits = []
        outpatient_benefits = []
        
        # Extract In-Patient Treatment section
        inpatient_match = re.search(r'In-Patient Treatment.*?\n(.*?)(?=Out-Patient|Exclusions|Emergency)', full_text, re.DOTALL | re.IGNORECASE)
        if not inpatient_match:
            # Try alternative patterns
            inpatient_match = re.search(r'Inpatient.*?Treatment.*?\n(.*?)(?=Outpatient|Exclusions|Emergency)', full_text, re.DOTALL | re.IGNORECASE)
        if not inpatient_match:
            inpatient_match = re.search(r'Hospital.*?Treatment.*?\n(.*?)(?=Outpatient|Exclusions|Emergency)', full_text, re.DOTALL | re.IGNORECASE)
        ...
        outpatient_match = re.search(r'Out-Patient.*?Treatment.*?\n(.*?)(?=Exclusions|Emergency|Pharmacy|In-Patient)', full_text, re.DOTALL | re.IGNORECASE)
        if not outpatient_match:
            # Try alternative patterns
            outpatient_match = re.search(r'Outpatient.*?Treatment.*?\n(.*?)(?=Exclusions|Emergency|Pharmacy)', full_text, re.DOTALL | re.IGNORECASE)
        if not outpatient_match:
            outpatient_match = re.search(r'OP.*?Treatment.*?\n(.*?)(?=Exclusions|Emergency|Pharmacy)', full_text, re.DOTALL | re.IGNORECASE)
        ...
        # Add structured categories if we found them (merge with existing benefits)
        if "benefits" not in base_plan_data:
            base_plan_data["benefits"] = []
        
        # Find existing inpatient/outpatient categories or create new ones
        existing_inpatient_idx = None
        existing_outpatient_idx = None
        
        for idx, category in enumerate(base_plan_data["benefits"]):
            if category.get("categoryId") == "inpatient":
                existing_inpatient_idx = idx
            elif category.get("categoryId") == "outpatient":
                existing_outpatient_idx = idx
        
        if inpatient_benefits:
            if existing_inpatient_idx is not None:
                # Merge with existing
                base_plan_data["benefits"][existing_inpatient_idx]["benefits"].extend(inpatient_benefits)
            else:
                # Create new category
        ...
        if outpatient_benefits:
            if existing_outpatient_idx is not None:
                # Merge with existing
                base_plan_data["benefits"][existing_outpatient_idx]["benefits"].extend(outpatient_benefits)
            else:
                # Create new category
                base_plan_data["benefits"].append({
        ...
        # Store PDF text snippet in raw data (same structure as Alsagr)
