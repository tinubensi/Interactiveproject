#!/usr/bin/env python3
"""
CLI wrapper for Takaful bot - called from Node.js Express server
...
from vendors.takaful.bot import TakafulBot
from vendors.takaful.adapter import TakafulAdapter
...
async def main():
    parser = argparse.ArgumentParser(description='Run Takaful bot')
    ...
    try:
        ...
        vendor_id = 'vendor-takaful'
        registry_vendor_id = 'takaful'
        ...
        if not credentials:
            raise ValueError(f"Takaful credentials not found for key: {vendor_id}")
        
        # Initialize adapter and prepare vendor payload
        adapter = TakafulAdapter()
        ...
        print(f"Starting Takaful bot for lead {lead_id}", file=sys.stderr)
        
        # Run bot using exact executor flow
        async with TakafulBot(credentials=credentials, config=bot_config) as bot:
