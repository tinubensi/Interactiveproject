#!/usr/bin/env python3
"""
CLI wrapper for Watania bot - called from Node.js Express server
...
from vendors.watania.bot import WataniaBot
from vendors.watania.adapter import WataniaAdapter
...
async def main():
    parser = argparse.ArgumentParser(description='Run Watania bot')
    ...
    try:
        ...
        vendor_id = 'vendor-watania'
        registry_vendor_id = 'watania'
        ...
        if not credentials:
            raise ValueError(f"Watania credentials not found for key: {vendor_id}")
        
        # Initialize adapter and prepare vendor payload
        adapter = WataniaAdapter()
        ...
        print(f"Starting Watania bot for lead {lead_id}", file=sys.stderr)
        
        # Run bot using exact executor flow
        async with WataniaBot(credentials=credentials, config=bot_config) as bot:
            ...
            raw_plans = await asyncio.wait_for(scraper.extract_all_plans(bot), timeout=300.0)
