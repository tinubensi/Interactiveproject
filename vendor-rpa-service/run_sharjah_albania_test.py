#!/usr/bin/env python3
"""
Test Sukoon bot with Sharjah emirate, Albania nationality, female, married, DOB 13-06-1996
"""
import asyncio
import json
import sys
...
async def main():
    ...
    try:
        ...
        async with SukoonBot(credentials=credentials, config=bot_config) as bot:
            ...
            await asyncio.wait_for(bot.fill_insurance_form(lead_data), timeout=120.0)
            ...
            output_file = Path(__file__).parent / f"sukoon-sharjah-albania-scraped-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
