...
exports.getAnalyticsOverview = exports.getWorkflowAnalytics = exports.getPeriodRange = void 0;
...
// Workflow Analytics
// ----------------------------------------------------------------------------
async function getWorkflowAnalytics(workflowId, period = 'week') {
    ...
    const stepMetrics = Array.from(stepMetricsMap.entries()).map(([stepId, data]) => ({
        stepId,
        stepName: data.name,
        stepType: data.type,
...
// Analytics Overview
// ----------------------------------------------------------------------------
async function getAnalyticsOverview(organizationId, period = 'week') {
    const containers = await (0, cosmosClient_1.getCosmosContainers)();
    const { start, end } = getPeriodRange(period);
    ...
    const activeWorkflows = Array.from(workflowMap.values()).filter((w) => w.status === 'active').length;
    // Get instances for the period
    const instancesQuery = {
        query: `
