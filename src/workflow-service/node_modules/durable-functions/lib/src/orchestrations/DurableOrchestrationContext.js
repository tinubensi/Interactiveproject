...
class DurableOrchestrationContext {
    ...
    callSubOrchestrator(name, input, instanceId, version) {
        if (!name) {
            throw new Error("A sub-orchestration function name must be provided when attempting to create a suborchestration");
        }
        const newAction = new CallSubOrchestratorAction_1.CallSubOrchestratorAction(name, instanceId, input, version);
        const task = new task_1.AtomicTask(false, newAction);
        return task;
    }
    callSubOrchestratorWithRetry(name, retryOptions, input, instanceId, version) {
        if (!name) {
            throw new Error("A sub-orchestration function name must be provided when attempting to create a suborchestration");
        }
        const newAction = new CallSubOrchestratorWithRetryAction_1.CallSubOrchestratorWithRetryAction(name, retryOptions, input, instanceId, version);
        const backingTask = new task_1.AtomicTask(false, newAction);
        const task = new task_1.RetryableTask(backingTask, retryOptions);
        return task;
    }
