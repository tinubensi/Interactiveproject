...
export function getCorsHeaders(request: HttpRequest): Record<string, string> {
  ...
  // If origin is not in allowed list, check if it's localhost, Azure Functions, or Vercel domain
  ...
  // Use the origin if it's allowed, localhost, Azure Functions, or Vercel domain
  const allowedOrigin = (isAllowed || isLocalhost || isAzureFunctions || isVercel) ? origin : ALLOWED_ORIGINS[0];
