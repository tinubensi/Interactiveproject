...
/**
 * HTTP endpoint to check and handle timeouts
 * GET /api/pipeline/check-timeouts
 * 
 * Checks all active instances for timeouts and auto-advances them
 * Can be called manually or by a scheduler (Azure Logic App, etc.)
 */
async function checkTimeoutsHandler(
  request: HttpRequest,
  context: InvocationContext
): Promise<HttpResponseInit> {
  const preflightResponse = handlePreflight(request);
  ...
  try {
    context.log('[TIMEOUT CHECK] Manual timeout check initiated');

    const { checkAndHandleTimeouts } = await import('../../lib/orchestrator');
    
    const result = await checkAndHandleTimeouts(context.log.bind(context));

    context.log(`[TIMEOUT CHECK] Complete: checked ${result.checked}, timed out ${result.timedOut}, handled ${result.handled}`);
    ...
    context.error('Error checking timeouts:', error);
    return errorResponse(request, String(error), 500);
  }
}

app.http('CheckTimeouts', {
  methods: ['GET', 'POST', 'OPTIONS'],
  authLevel: 'anonymous',
  route: 'pipeline/check-timeouts',
  handler: checkTimeoutsHandler,
});
