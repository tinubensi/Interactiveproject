...
describe('Data Pre-fill Service', () => {
  ...
  it('should pre-fill member details from lead data', () => {
    const prefilled = dataPrefillService.prefillFromLeadAndEmaf(sampleLeadData);
    
    assert.strictEqual(prefilled.memberDetails.applicantName, 'John Doe');
    assert.strictEqual(prefilled.memberDetails.email, 'john.doe@example.com');
    assert.strictEqual(prefilled.memberDetails.contactNumber, '+971501234567');
    assert.strictEqual(prefilled.memberDetails.address, 'Dubai');
    assert.strictEqual(prefilled.memberDetails.emiratesId, '784-1990-1234567-1');
    assert.strictEqual(prefilled.memberDetails.occupation, 'Engineer');
    assert.strictEqual(prefilled.memberDetails.poBox, '12345');
    assert.strictEqual(prefilled.memberDetails.sponsorName, 'ABC Company');
  });
  ...
  it('should map salary correctly', () => {
    ...
    const prefilledLow = dataPrefillService.prefillFromLeadAndEmaf(lowSalaryLead);
    const prefilledHigh = dataPrefillService.prefillFromLeadAndEmaf(highSalaryLead);
  ...
  it('should handle missing lead data gracefully', () => {
    const prefilled = dataPrefillService.prefillFromLeadAndEmaf(undefined);
  ...
  it('should handle partial lead data', () => {
    ...
    const prefilled = dataPrefillService.prefillFromLeadAndEmaf(partialLead);
  ...
  it('should merge existing EMAF data with lead data', () => {
    ...
    const prefilled = dataPrefillService.prefillFromLeadAndEmaf(sampleLeadData, existingEmafData);
  ...
  it('should set default relationship to Self', () => {
    ...
    const prefilled = dataPrefillService.prefillFromLeadAndEmaf(leadWithoutRelationship);
  ...
  it('should handle phone number with country code', () => {
    ...
    const prefilled = dataPrefillService.prefillFromLeadAndEmaf(leadWithPhone);
  ...
  it('should pre-fill from multiple sources with customer profile override', () => {
    const customerProfile = {
      fullName: 'Customer Profile Name',
